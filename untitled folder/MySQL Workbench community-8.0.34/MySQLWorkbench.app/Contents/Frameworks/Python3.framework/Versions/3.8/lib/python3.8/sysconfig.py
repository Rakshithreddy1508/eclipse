...
if os.environ.get("_PYTHON_PROJECT_BASE", "").strip() != "":
...
def _parse_makefile(filename, vars=None):
    ...
    for_xcode = done.get('TRAIN_STYLE') == 'DT'

    # make sure we use the public macosx SDK
    if for_xcode:
        import re
        for k, v in done.items():
            if isinstance(v, str):
                v = re.sub(r'macosx[\.\d]+internal', 'macosx', v)
                v = re.sub(r'^xcrun -sdk macosx clang', 'clang', v)
                done[k] = v
